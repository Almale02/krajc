...
pub mod prelude {
    ...
    pub use crate::engine_runtime::schedule_manager::schedule::IntoSystem;
    pub use crate::engine_runtime::schedule_manager::system_params::system_engine::dump_memory;
    pub use crate::run;
    pub use crate::span;
    pub use bevy_ecs;
...
use crate::rendering::{
    builtin_materials::texture_material::material::update_texture_material, managers::TargetFps,
    systems::general::update_rendering,
};
...
use engine_runtime::{
    input::{KeyboardInput as KeyInput, MouseInput},
    schedule_manager::{
        runtime_schedule::RuntimePostUpdateSchedule,
        schedule::{IntoSystem, Schedule as _, ScheduleRunnable},
        system_params::{
            system_engine::dump_memory, system_param::FunctionSystem, system_query::SystemQuery,
        },
    },
};
use engine_runtime::{
...
use rendering::{
    ...
    builtin_materials::{
        light_material::material::{update_light_material, LightMaterial, LightMaterialResource},
        texture_material::material::{TextureMaterial, TextureMaterialResource},
    },
    camera::camera::Camera,
    lights::{LightLookDirText, PointLight, SpotLight},
    managers::{new::RenderFeatures, RenderManagerResource},
...
pub static mut CUSTOM_SYSTEM: TypedAddr<Takeable<Box<dyn ScheduleRunnable>>> = TypedAddr::default();
...
fn startup(
    ...
    render_features: Res<RenderFeatures>,
    mut text_producer: Res<DebugTextProducer>,
) {
    let supports_lighting = render_features.dynamic_arrays;
    ...
    world.spawn((text_producer.create_text(""), MouseMotionText));
    for y in 0..128 {
        for x in 0..128 {
            let x = x - 64;
            let y = y - 64;
...
fn update_player_pos_text(
    ...
    mouse_text.text = format!(
        "mouse motion x: {:.2}, y: {:.2}",
        mouse_input.get_mouse_motion().0,
        mouse_input.get_mouse_motion().1
    );
}
...
pub async fn run(game: SystemPlugin) {
    ...
    let render_features = runtime.get_resource::<RenderFeatures>();

    //if render_features.dynamic_arrays {}
    /*runtime
    .get_resource_mut::<LightMaterialResource>()
    .shader_asset_handle = runtime
    .asset_manager
    .load_resource(
    ...
    let test_leak = Box::leak(Box::new(4_u32));

    //dump_memory(runtime);
    (game.register_systems)(
        TypedAddr::new_with_ref(runtime),
        SystemPluginRegister::new(TypedAddr::new_with_ref(runtime), unsafe {
            &mut CUSTOM_SYSTEM
        }),
        TypedAddr::new_with_ref(test_leak),
    );
    dbg!(test_leak);
    ...
    loop {
        ...
        while let Some(x) = controller.next_event() {
            match x.event {
                ...
                gilrs::EventType::ButtonChanged(button, value, code) => {
                    /*println!(
                        "button {:?} with code {:?} was changed to {}",
                        button, code, value
                    );*/
                }
                gilrs::EventType::AxisChanged(axis, value, code) => {
                    // println!(
                    //     "axis {:?} with code {:?} changed to value {}",
                    //     axis, code, value
                    // );
                }
        ...
        //dbg!(runtime.test_map.get(&4));
        //println!("typeid from engine: {:?}", RuntimeUpdateSchedule::uuid());
